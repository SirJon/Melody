...
import { domPopupCertificate } from "../popup/render/domPopupCertificate";
import { sendRequest } from "../sendRequest/sendRequest.js";

const IMG_WIDTH = 1280;

const body = document.querySelector(`body`);
body.appendChild(domPopupCertificate);

export const popup = new popupConstructor({
  selector: `.popup--certificate`,
  overflow: true,
});

const closeButton = document.querySelector(`.popup--certificate .popup__button--close`);
const mask = document.querySelector(`.popup--certificate`);
...
sendRequest('GET', dirURL)
  .then(data => {
    ...
    sendRequest('GET', qualityURL)
      .then(data => {
        const img = data.productCertification;
